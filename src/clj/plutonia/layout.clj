   ...
   (fn []
     [:html {:lang "en"}
      [:head
       ...
       [:meta {:name "viewport"
               :content "width=device-width, initial-scale=1, shrink-to-fit=no"}]
       [:title "Something bad happened - plutonia"]
       [:meta {:name "description" :content "Error page"}]

       [:link {:href "css/plutonia/main.min.css" :rel "stylesheet" :type "text/css"}]
      ...
      [:body {:class "container themable framing-shadow sticky-footer-container"}
       (basic-skeleton "plutonia" ["error" (str (:status error-details))]
                       "img/foggy-shit-small.jpg"
                       (:title error-details)
                       "main-error")]])))
