(ns plutonia.instagram.events
  (:require
    [reagent.core :as r]
    [re-frame.core :as rf]
    [ajax.core :as ajax]
    [plutonia.util :as util]
    [plutonia.cofx :as cofx]
    [plutonia.interceptors :as inter]
    ...
    :dispatch [:redirect-to-uri-well-actually-no-popup-a-link
               {:uri "https://www.instagram.com/oauth/authorize"
                :url-params {:client_id (:client_id secrets)
                             :redirect_uri "https://plutonia.se"
                             :scope "user_profile,user_media"
                             :response_type "code"}}]}))

; ^ not how it works we need to redirect to that url then redirects back and capture the query
; so would change redir url to like plutonia.se/redir/instagram and have controller there
